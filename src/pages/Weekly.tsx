    ...
    <div className="min-h-screen pb-24 max-w-md mx-auto w-full" style={{ backgroundColor: '#F4F1EA' }}>
      ...
      {members.length > 0 && (
        <div className="px-4 py-2 flex gap-2 overflow-x-auto">
          <button
            ...
            style={selectedMemberFilter === null ? { backgroundColor: '#5C6B4A' } : { backgroundColor: '#FDFBF7' }}
          >
            All
          </button>
          {members.map((m) => (
            <button
              ...
              className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-medium transition-colors flex items-center gap-1.5 ${
                selectedMemberFilter === m.id ? 'text-white' : 'border border-beige-300 text-neutral-600'
              }`}
              style={selectedMemberFilter === m.id ? { backgroundColor: '#5C6B4A' } : { backgroundColor: '#FDFBF7' }}
            >
              <span
      ...
      <section className="px-4 py-3 flex items-center justify-between">
        ...
        <div className="text-center">
          <p className="font-serif font-semibold text-xl" style={{ color: '#2D3319' }}>This Week</p>
          <p className="text-xs mt-0.5" style={{ color: '#6B7B5E' }}>{weekRangeLabel}</p>
        </div>
        <button
          type="button" onClick={() => setWeekOffset((o) => o + 1)}
          className="w-12 h-12 min-w-[48px] min-h-[48px] rounded-full flex items-center justify-center hover:bg-olive-50 border border-beige-300 transition-colors"
      ...
      {loading ? (
        ...
        <main className="text-center py-12 px-4">
          ...
          <button type="button" onClick={() => window.location.reload()} className="mt-3 px-4 py-2 text-white rounded-full text-sm" style={{ backgroundColor: '#5C6B4A' }}>
            Try Again
          </button>
        </main>
      ) : totalMeals === 0 ? (
        <main className="px-4 py-8 flex flex-col items-center justify-center text-center">
          ...
          <p className="font-serif text-lg font-semibold mb-2" style={{ color: '#2D3319' }}>No meals tracked this week yet</p>
          <p className="text-sm mb-6" style={{ color: '#6B7B5E' }}>Start scanning to see your family&apos;s nutrition progress!</p>
          <button onClick={() => navigate('/dashboard')} className="w-full max-w-xs py-3.5 rounded-full font-semibold text-white" style={{ backgroundColor: '#5C6B4A' }}>
        ...
        <main className="px-4 py-4 space-y-5">
          ...
          <section className="rounded-2xl p-5" style={{ backgroundColor: '#FDFBF7' }}>
            <h2 className="font-serif font-semibold mb-3" style={{ color: '#2D3319' }}>Daily Calories</h2>
                ...
                return (
                  <div key={i} className="flex flex-col items-center flex-1">
                    ...
                    <div
                      ...
                      style={{
                        height: `${height}%`,
                        backgroundColor: isToday ? '#5C6B4A' : '#8B9E6B',
                        opacity: cal > 0 ? 1 : 0.3,
                        minHeight: 4,
                      }}
                    />
                    <span className={`text-[10px] mt-1 ${isToday ? 'font-bold' : ''}`} style={{ color: '#6B7B5E' }}>
                      {DAY_LABELS[i]}
                    </span>
                  </div>
                );
            ...
            <p className="text-center text-xs mt-3" style={{ color: '#6B7B5E' }}>
          ...
          <section className="grid grid-cols-2 gap-3">
            <div className="rounded-2xl p-4" style={{ backgroundColor: '#FDFBF7' }}>
              <p className="text-xs font-medium text-neutral-500 mb-1">Avg Calories</p>
              <p className="text-2xl font-bold" style={{ color: '#2D3319' }}>{avgCalories}</p>
              <p className="text-xs" style={{ color: '#6B7B5E' }}>cal/meal</p>
            </div>
            <div className="rounded-2xl p-4" style={{ backgroundColor: '#FDFBF7' }}>
              <p className="text-xs font-medium text-neutral-500 mb-1">Top Dish</p>
              <p className="text-sm font-semibold" style={{ color: '#2D3319' }}>
                {topDish ? `${topDish.name}` : '—'}
              </p>
              {topDish && (
                <p className="text-xs" style={{ color: '#6B7B5E' }}>{topDish.count} times</p>
              )}
            </div>
          </section>

          {/* SECTION 5: Daily Meal Grid */}
          <section className="rounded-2xl shadow-sm p-4" style={{ backgroundColor: '#FDFBF7' }}>
            <h2 className="font-serif font-semibold mb-3" style={{ color: '#2D3319' }}>Daily Meals</h2>
                ...
                return (
                  <div key={day} className="flex items-start gap-3 py-2 border-b last:border-0" style={{ borderColor: '#e8e4dc' }}>
                    <div className="w-14 flex-shrink-0">
                      <p className="text-sm font-semibold" style={{ color: '#2D3319' }}>{day}</p>
                    </div>
          ...
          {suggestions.length > 0 ? (
            <div className="mt-2">
              <h3 className="font-serif text-lg font-semibold mb-3" style={{ color: '#2D3319' }}>Try This Week</h3>
              <p className="text-xs mb-3" style={{ color: '#6B7B5E' }}>Based on your family&apos;s nutrition this week</p>
              <div className="space-y-3">
                {suggestions.map((recipe, i) => (
                  <div key={i} className="p-4 rounded-2xl border border-gray-100" style={{ backgroundColor: '#FDFBF7' }}>
                    <div className="flex items-start gap-3">
                      <span className="text-3xl">{recipe.emoji}</span>
                      <div className="flex-1">
                        <p className="font-semibold" style={{ color: '#2D3319' }}>{recipe.name}</p>
                        <p className="text-xs mb-1" style={{ color: '#6B7B5E' }}>{recipe.telugu} • ⏱️ {recipe.prep}</p>
                        <p className="text-sm" style={{ color: '#5C6B4A' }}>{recipe.why}</p>
                      </div>
                    </div>
                  </div>
                ))}
            ...
            <div className="mt-2 p-4 bg-emerald-50 rounded-2xl text-center">
              <span className="text-3xl">🎉</span>
              <p className="font-serif mt-2 font-semibold" style={{ color: '#5C6B4A' }}>Great balance this week!</p>
          ...
          <section className="flex flex-col gap-3">
            <button type="button" onClick={() => navigate('/monthly')} className="w-full py-3.5 rounded-full font-semibold border-2 transition-colors hover:bg-olive-50" style={{ borderColor: '#5C6B4A', color: '#5C6B4A' }}>
              View Monthly Overview
            </button>
            <button type="button" onClick={() => navigate('/grocery')} className="w-full py-3.5 rounded-full font-semibold text-white" style={{ backgroundColor: '#5C6B4A' }}>
              Generate Grocery List
            </button>
            <button type="button" onClick={() => navigate('/dashboard')} className="w-full py-3.5 rounded-full font-semibold border-2 transition-colors hover:bg-olive-50" style={{ borderColor: '#5C6B4A', color: '#5C6B4A' }}>
              Scan a Meal
            </button>
          </section>
        </main>
